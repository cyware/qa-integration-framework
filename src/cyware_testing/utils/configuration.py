...
from cyware_testing import DATA_PATH
from cyware_testing.constants.paths.configurations import CYWARE_CONF_PATH, CYWARE_LOCAL_INTERNAL_OPTIONS

from . import file


def get_minimal_configuration():
    """Get the cyware minimal configuration data.

    Returns:
        List of str: Cyware minimal configuration data.
    """
    return file.read_file_lines(os.path.join(DATA_PATH, 'all_disabled_ossec.conf'))


def get_cyware_conf() -> List[str]:
    """
    ...
    """
    return file.read_file_lines(CYWARE_CONF_PATH)


def write_cyware_conf(cyware_conf: List[str]) -> None:
    """
    Write a new configuration in 'ossec.conf' file.

    Args:
        cyware_conf (list or str): Lines to be written in the ossec.conf file.
    """
    file.write_file(CYWARE_CONF_PATH, cyware_conf)


def set_section_cyware_conf(sections: List[dict], template: List[str] = None) -> List[str]:
    """
    Set a configuration in a section of Cyware. It replaces the content if it exists.

    Args:
        sections (list): List of dicts with section and new elements
        section (str, optional): Section of Cyware configuration to replace. Default `'syscheck'`
    ...
    def find_module_config(cyware_conf: ET.ElementTree, section: str, attributes: List[dict]) -> ET.ElementTree:
        r"""
        ...
        Args:
            cyware_conf (ElementTree): An ElementTree object with all the data of the ossec.conf
        ...
        if attributes is None:
            return cyware_conf.find(section)
        else:
            ...
            try:
                return cyware_conf.find(query)
            except AttributeError:
                return None

    # Get Cyware configuration as a list of str
    raw_cyware_conf = get_cyware_conf() if template is None else template
    # Generate a ElementTree representation of the previous list to work with its sections
    cyware_conf = to_elementTree(purge_multiple_root_elements(raw_cyware_conf))
    for section in sections:
        attributes = section.get('attributes')
        section_conf = find_module_config(
            cyware_conf, section['section'], attributes)
        # Create section if it does not exist, clean otherwise
        if not section_conf:
            section_conf = ET.SubElement(
                cyware_conf.getroot(), section['section'])
            section_conf.text = '\n    '
            section_conf.tail = '\n\n  '
        else:
    ...
    return to_str_list(cyware_conf)
...
def get_local_internal_options_dict():
    ...
    with open(CYWARE_LOCAL_INTERNAL_OPTIONS, 'r') as local_internal_option_file:
...
def set_local_internal_options_dict(dict_local_internal_options):
    ...
    """
    with open(CYWARE_LOCAL_INTERNAL_OPTIONS, 'w') as local_internal_option_file:
...
def load_configuration_template(data_file_path, configuration_parameters=[], configuration_metadata=[]):
    """Load different configurations of Cyware from a YAML file.
    ...
    Returns:
        list(dict): List containing cyware configurations in dictionary form.
