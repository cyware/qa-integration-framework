...
from cyware_testing import session_parameters
from cyware_testing.constants.api import CYWARE_API_PROTOCOL, CYWARE_API_HOST, CYWARE_API_PORT, CYWARE_API_USER, \
                                        CYWARE_API_PASSWORD, LOGIN_ROUTE, USERS_ROUTE, RESOURCE_ROUTE_MAP, \
                                        TARGET_ROUTE_MAP
from cyware_testing.constants.paths.api import CYWARE_API_CERTIFICATE
from cyware_testing.modules.api.patterns import API_LOGIN_ERROR_MSG
from cyware_testing.utils.file import read_json_file
...
def get_base_url(protocol: str = CYWARE_API_PROTOCOL, host: str = CYWARE_API_HOST, port: str = CYWARE_API_PORT) -> str:
    """Get Cyware API's base URL.
    ...
    Returns:
        base_url (str): Cyware API's base URL
    """
...
def set_authorization_header(user: str = None, password: str = None) -> dict:
    ...
    """
    user = CYWARE_API_USER if user is None else user
    password = CYWARE_API_PASSWORD if password is None else password
...
def login(user: str = CYWARE_API_USER, password: str = CYWARE_API_PASSWORD,
          timeout: int = session_parameters.default_timeout, login_attempts: int = 3, backoff_factor: float = 0.5,
          host: str = CYWARE_API_HOST, port: str = CYWARE_API_PORT, protocol: str = CYWARE_API_PROTOCOL
    ...
    response = session.post(url, headers=set_authorization_header(user, password), verify=CYWARE_API_CERTIFICATE,
                                timeout=timeout)
...
def allow_user_to_authenticate(user_id: str = None) -> requests.Response:
    ...
    response = requests.put(url + params, headers=authentication_headers, verify=CYWARE_API_CERTIFICATE)
...
def manage_security_resources(method: str = 'get', resource: Union[dict, str] = None,
    ...
    authentication_headers = login()[0]
    response = getattr(requests, method)(url, headers=authentication_headers, verify=CYWARE_API_CERTIFICATE, json=payload)
...
def relate_resources(test_metadata: dict) -> None:
    ...
    for origin in relationships:
        ...
        for idx, target_value in enumerate(target_values):
            ...
            response = requests.post(url, headers=login()[0], verify=CYWARE_API_CERTIFICATE)
            if response.status_code != 200 or response.json()['error'] != 0:
...
def remove_resources_relationship(origin_resource: dict = None, target_resource: dict = None) -> dict:
    ...
    response = requests.delete(url, headers=login()[0], verify=CYWARE_API_CERTIFICATE)
    if response.status_code != 200 or response.json()['error'] != 0:
