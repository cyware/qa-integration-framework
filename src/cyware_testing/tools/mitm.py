...
from cyware_testing.constants.users import CYWARE_UNIX_USER, CYWARE_UNIX_GROUP
...
class StreamHandler(socketserver.BaseRequestHandler):
    ...
    def default_cyware_handler(self):
        """Default cyware daemons TCP handler method for MITM server."""
...
class DatagramHandler(socketserver.BaseRequestHandler):
    ...
    def default_cyware_handler(self):
        """Default cyware daemons UDP handler method for MITM server."""
        data = self.request[0]
...
class ManInTheMiddle:
    ...
    def run(self, *args):
        ...
        if isinstance(self.listener_socket_address, str):
            uid = pwd.getpwnam(CYWARE_UNIX_USER).pw_uid
            gid = grp.getgrnam(CYWARE_UNIX_GROUP).gr_gid
